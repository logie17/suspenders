...
#+begin_src emacs-lisp
  ...
  (defun logan/create-or-open-todays-standup()
    ...
    (let ((firstLine (thing-at-point 'line t)))
      (when (not (string-match "\* yesterday" firstLine))
        (insert "* yesterday\n\n* today\n")
        (goto-line 2)
        (insert "** ")))
    (split-window-below)
    (let ((yesterday (time-subtract (current-time) (* 24 3600))))
          (find-file (format "~/work/frameable/standup/%s.org" (format-time-string "%m%d%y" yesterday)))))
